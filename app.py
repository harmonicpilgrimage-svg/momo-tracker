...
def api_sms():
    ...
    # Parse directly - we trust the phone-side filter
    parsed = parse_momo_sms(sms_text)
    if not parsed["amount"]:
        return jsonify({"success": False, "error": "No amount found", "raw": sms_text[:200]})
    txn_type = parsed["type"] if parsed["type"] != "UNKNOWN" else "SEND"
    txn = add_transaction(txn_type, parsed["phone"] or "N/A", parsed["amount"], f"sms-auto-{parsed.get('date','')}-{parsed.get('time','')}", "SUCCESSFUL")

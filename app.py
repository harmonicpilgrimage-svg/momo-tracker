...
<body>
    <div class="header">
        ...
        <div class="subtitle">Your number: {{ my_number }}</div>
    </div>
    ...
    <div class="stats">
        ...
        <div class="stat-card"><div class="stat-value">{{ stats.total_sent }} EUR</div><div class="stat-label">Total Sent</div></div>
        <div class="stat-card"><div class="stat-value">{{ stats.total_received }} EUR</div><div class="stat-label">Total Received</div></div>
    </div>
    ...
    <script>
        ...
        async function checkBalance() {
            ...
            try {
                ...
                const data = await resp.json();
                showToast(data.success ? 'Balance: ' + (data.balance || 'N/A') + ' ' + (data.currency || 'EUR') : 'Could not get balance (sandbox mode)');
    ...
    try:
        ...
        if transactions:
            ...
            for txn in transactions:
                txn_type_display = "📤 SENT" if txn["type"] == "SEND" else "📥 REQUESTED"
                txn_type_class = "send" if txn["type"] == "SEND" else "request"
                note_html = ""
                if txn.get("note"):
                ...
                <div class="txn-top">
                    <span class="txn-type {txn_type_class}">{txn_type_display}</span>
                    <span class="txn-status status-{txn['status']}">{txn['status']}</span>
                </div>
                <div class="txn-amount">{txn['amount']} EUR</div>
            ...
            txn_html = '<div class="empty-state"><div class="icon">📭</div><p>No transactions yet!</p><p style="font-size:13px;">Send money or request a payment to get started.</p></div>'

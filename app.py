...
def api_sms():
    data = request.get_json(silent=True) or {}
    sms_text = data.get("sms", data.get("text", request.get_data(as_text=True)))
    if not sms_text: return jsonify({"success": False, "error": "No SMS text"}), 400
    if not is_momo_sms(sms_text): return jsonify({"success": False, "detected": False})
    parsed = parse_momo_sms(sms_text)
    if not parsed["amount"]: return jsonify({"success": False, "error": "Could not parse"})
    txn_type = parsed["type"] if parsed["type"] != "UNKNOWN" else "SEND"
    txn = add_transaction(txn_type, parsed["phone"] or "N/A", parsed["amount"], f"sms-{parsed.get('date','')}-{parsed.get('time','')}", "SUCCESSFUL")
    return jsonify({"success": True, "detected": True, "transaction_id": txn["id"], "message": f"Detected {txn_type} of {parsed['amount']} RWF!"})
